...
import { UserPlus, Mail, User, Upload, FileText, Activity, Check, ChevronLeft } from 'lucide-react';
...
export default function AddClientPage() {
  ...
  const handleSubmit = async (e: any) => {
    ...
    try {
      ...
      // 1. Create auth user via signUp (coach stays logged in after)
      ...
      if (!coach) throw new Error('Coach session lost.');

      // 2. Create client_profile row
      const { error: cpError } = await supabase.from('client_profiles').insert({
        ...
        workouts_per_week: parseInt(formData.workouts_per_week as any),
      ...
      if (inbodyFile) {
        ...
        await supabase.from('inbody_scans').insert({
          ...
          weight: inbody.weight || 0,
          smm: inbody.smm || 0,
          bfm: inbody.bfm || 0,
          bf_percent: inbody.bf_percent || 0,
          bmr: inbody.bmr || 0,
          inbody_score: inbody.inbody_score || 0
        });
      }

      // 4. Create workout day slots
      const days = [];
      for (let i = 1; i <= parseInt(formData.workouts_per_week as any); i++) {
        days.push({ user_id: userId, day_number: i, day_name: `Day ${i}`, exercises: [] });
      }
      await supabase.from('client_workout_days').insert(days);

      toast.success(`✅ Client created! Passcode: ${passcode}`, { duration: 8000 });
